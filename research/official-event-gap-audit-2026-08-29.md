...
2026年8月31日までに、特設ページ系列99件を`research/official-special-events-source.json`へ追加し、公開JSONへ統合した。2026年9月1日時点では`data/official-spots.json`70件、`data/official-events-archive.json`484件、`data/nagano-spots.json`91件の合計645件である。特設ページのうち会場・開始日・終了日を確定できた系列は収録済みだが、水族館と東京みやげは掲載単位または開始日を確定できないため保留している。したがって、484件も「確認済みの公式一覧＋会場別日程を確認できた特設ページ系列」の件数であり、公式企画すべてを無条件に地図化した件数とは扱わない。
...
水族館2ページと東京みやげの公式ページを再確認したが、今回は公開JSONへ追加していない。

- 水族館第1弾は仙台うみの杜水族館の水槽展示、第2弾は同館のフォトスポット・ARラリーを確認できる。一方で、各会場は「取扱店舗」として掲載され、会場別の終了日は確認できない。商品取扱いのみの会場も混在するため、推測で期間を設定しない。
- 東京みやげは羽田空港第2ターミナル Tokyo's Tokyoだけ開始日と終了日の両方を確認できるが、独立した催事ではなく既存店舗での商品取扱いに該当する。他会場の多くは開始日がないため、現在の掲載基準では登録しない。

確認結果は`official-special-series-catalog.json`へ記録した。以後は公式総合ページの確認時に台帳を更新し、`pnpm run audit:special-series`で特設イベント原本との差分を確認する。

## 次回確認時の手順

1. 公式総合ページで、前回台帳にない国内実会場の系列を探す
2. `official-special-series-catalog.json`へ対応状況と理由を記録する
3. 掲載対象だけ会場・期間・住所を確認し、`official-special-events-source.json`へ追加する
4. `pnpm run audit:special-series`と`pnpm run check`を実行する
